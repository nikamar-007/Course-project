    ...
    try {
        $stmt = $pdo->prepare("
            SELECT
              ...
              rz.creator_id,
              rz.status,
              -- Определяем цвет согласно вашей логике
              CASE 
                -- Pending: 0 подтверждений, <24 часов (серая, но в запросе мы ее не показываем)
                WHEN rz.confirm_count = 0 AND rz.created_at >= NOW() - INTERVAL '24 hours' 
                     THEN '#9E9E9E'
                
                -- Active Red: ≥1 подтверждение, ≤7 дней с создания
                WHEN rz.confirm_count > 0 AND rz.created_at >= NOW() - INTERVAL '7 days' 
                     THEN '#E53935'
                
                -- Active Yellow: ≥1 подтверждение, 7-14 дней с создания
                WHEN rz.confirm_count > 0 AND rz.created_at >= NOW() - INTERVAL '14 days' 
                     THEN '#FBC02D'
                
                -- Expired Green: >14 дней с создания, но жива из-за подтверждений
                WHEN rz.confirm_count > 0 AND rz.created_at < NOW() - INTERVAL '14 days' 
                     AND rz.last_confirmed_at >= NOW() - INTERVAL '24 hours'
                     THEN '#43A047'
                
                -- Если не подошло ни под одно условие, значит зона мертва
                ELSE NULL
              END AS color
            ...
            WHERE rz.is_active = true
              ...
              -- Показываем только зоны, которые должны быть видны
              AND (
                -- Pending зоны (только что созданные, еще не подтвержденные)
                (rz.confirm_count = 0 AND rz.created_at >= NOW() - INTERVAL '24 hours')
                OR
                -- Active Red (≤7 дней с создания)
                (rz.confirm_count > 0 AND rz.created_at >= NOW() - INTERVAL '7 days')
                OR
                -- Active Yellow (7-14 дней с создания)
                (rz.confirm_count > 0 AND rz.created_at >= NOW() - INTERVAL '14 days' 
                 AND rz.created_at < NOW() - INTERVAL '7 days')
                OR
                -- Expired Green (>14 дней, но подтверждена в последние 24 часа)
                (rz.confirm_count > 0 AND rz.created_at < NOW() - INTERVAL '14 days'
                 AND rz.last_confirmed_at >= NOW() - INTERVAL '24 hours')
              )
            ORDER BY 
              CASE 
                WHEN rz.confirm_count = 0 THEN 0  -- Pending зоны в конце
                ELSE 1
              END,
    ...
    try {
        ...
        $sql = "
            ...
            VALUES (
                ...
                :creator_id,
                NOW(),
                NULL,  -- last_confirmed_at пока NULL (нет подтверждений)
                0,     -- confirm_count = 0
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    try {
        $zoneId = $_GET['id'] ?? null;
        if (!$zoneId) {
            throw new Exception('Не указан ID зоны');
        }
        ...
        $checkCreatorStmt = $pdo->prepare("
            SELECT creator_id, status 
            FROM repair_zones 
            WHERE id = :zone_id AND is_active = true
        ");
        $checkCreatorStmt->execute([':zone_id' => $zoneId]);
        $zone = $checkCreatorStmt->fetch();
        ...
        if ($zone['creator_id'] != $userId) {
            throw new Exception('Вы не являетесь создателем этой зоны');
        }
